...
static void SortRankings(void)
{
    ...
    for (k = 0; k < MAX_SCORES; k++)
    {
        ...
        attenuation = (rank->rounds < 300) ? ((double)rank->rounds / 300.0) : 1.0;

        kills = rank->kills;
        sc = (double)rank->score * attenuation;
        kd = ((rank->deaths != 0) ? (kills / (double)rank->deaths) : (kills)) * attenuation;
        kr = ((rank->rounds != 0) ? (kills / (double)rank->rounds) : (kills)) * attenuation;
...
void Rank_show_ranks(void)
{
    ...
    for (i = 0; i < MAX_SCORES; i++)
    {
        ...
        snprintf(tmpbuf, sizeof(tmpbuf),
                 (options.survivalScore == 0.0) ? "%d. %s (%.2f)" : "%d. %s (%.1f)",
...
void Rank_write_rankfile(void)
{
    ...
    for (i = 0; i < rank_entries; i++)
    {
        ...
        if (rank->score != 0.0 && fprintf(file, "score=\"%.2f\" ", rank->score) < 0)
            goto writefailed;

        if (rank->kills > 0 && fprintf(file, "kills=\"%d\" ", rank->kills) < 0)
            goto writefailed;

        if (rank->deaths > 0 && fprintf(file, "deaths=\"%d\" ", rank->deaths) < 0)
            goto writefailed;

        if (rank->rounds > 0 && fprintf(file, "rounds=\"%d\" ", rank->rounds) < 0)
            goto writefailed;

        if (rank->shots > 0 && fprintf(file, "shots=\"%d\" ", rank->shots) < 0)
            goto writefailed;

        if (rank->deadliest > 0 && fprintf(file, "deadliest=\"%d\" ", rank->deadliest) < 0)
            goto writefailed;

        if (rank->ballsCashed > 0 && fprintf(file, "ballscashed=\"%d\" ", rank->ballsCashed) < 0)
            goto writefailed;

        if (rank->ballsSaved > 0 && fprintf(file, "ballssaved=\"%d\" ", rank->ballsSaved) < 0)
            goto writefailed;

        if (rank->ballsWon > 0 && fprintf(file, "ballswon=\"%d\" ", rank->ballsWon) < 0)
            goto writefailed;

        if (rank->ballsLost > 0 && fprintf(file, "ballslost=\"%d\" ", rank->ballsLost) < 0)
            goto writefailed;

        if (rank->bestball > 0 && fprintf(file, "bestball=\"%.2f\" ", rank->bestball) < 0)
            goto writefailed;
