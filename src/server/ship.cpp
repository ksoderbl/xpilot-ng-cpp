...
void Make_thrust_sparks(Player *pl)
{
    ...
    min_dir = (int)(pl->dir + ANGLE_RESOLUTION / 2 - (ANGLE_RESOLUTION * 0.2 + 1) * options.thrustWidth);
...
void Delta_mv_elastic(object_t *obj1, object_t *obj2)
{
    ...
    obj1->vel.x = (m1 - m2) / ms * v1x + 2 * m2 / ms * v2x;
...
void Delta_mv_partly_elastic(object_t *obj1, object_t *obj2, double elastic)
{
    ...
    obj1->vel.x = ((m1 - m2) / ms * v1x + 2 * m2 / ms * v2x) * elastic + vx * (1 - elastic);
    obj1->vel.y = ((m1 - m2) / ms * v1y + 2 * m2 / ms * v2y) * elastic + vy * (1 - elastic);
    obj2->vel.x = (2 * m1 / ms * v1x + (m2 - m1) / ms * v2x) * elastic + vx * (1 - elastic);
    obj2->vel.y = (2 * m1 / ms * v1y + (m2 - m1) / ms * v2y) * elastic + vy * (1 - elastic);
...
void Obj_repel(object_t *obj1, object_t *obj2, int repel_dist)
{
    ...
    a = findDir(xd, yd);
    obj_theta = MOD2((int)(a + 0.5), ANGLE_RESOLUTION);
...
void Tank_handle_detach(Player *pl)
{
    ...
    // strlcpy(tank->name.c_str(), pl->name.c_str(), MAX_CHARS);
    // strlcat(tank->name.c_str(), "'s tank", MAX_CHARS);
    ...
    {
        Player *pl_i = Player_by_index(i + spectatorStart);
                 ...
                 double min_life, double max_life)
{
    ...
    for (i = 0; i < num_debris; i++)
    {
        ...
        debris->acc.x = 0;
        debris->acc.y = 0;
        if (options.shotHitFuelDrainUsesKineticEnergy && type == OBJ_SHOT)
        {
