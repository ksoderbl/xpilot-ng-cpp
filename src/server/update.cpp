...
static void Players_turn(void)
{
    ...
    for (i = 0; i < NumPlayers; i++)
    {
        ...
        while (new_float_dir < 0)
            new_float_dir += ANGLE_RESOLUTION;
        while (new_float_dir >= ANGLE_RESOLUTION)
            new_float_dir -= ANGLE_RESOLUTION;
...
static void Update_players(void)
{
    ...
    for (i = 0; i < NumPlayers; i++)
    {
        ...
        if (Player_is_alive(pl) && !BIT(pl->used, USES_SHIELD))
        {
        ...
        else
        {
            ...
            if (options.constantSpeed)
            {
                ...
                if (pl->last_wall_touch != frame_loops)
                { /* no bounce */
                    pl->vel.x -= options.constantSpeed * pl->acc.x;
                    pl->vel.y -= options.constantSpeed * pl->acc.y;
                }
...
void Update_objects(void)
{
    ...
    for (i = NumPlayers - 1; i >= 0; i--)
    {
        ...
        if (Player_is_paused(pl))
        {
            ...
            if (Player_is_human(pl) && options.maxPauseTime > 0 && pl->pauseTime > options.maxPauseTime)
            {
