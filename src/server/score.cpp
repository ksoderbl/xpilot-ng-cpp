                   ...
                   double loser_score, char *loser_msg, bool transfer_tag)
{
    ...
    if (options.tagGame && winner_score > 0.0 && loser_score < 0.0 && transfer_tag)
    {
                    ...
                    void *extra, const char *somemsg)
{
    ...
    case SCORE_TARGET:
    {
        ...
        for (j = 0; j < NumPlayers; j++)
        {
            ...
            if (pl->team == targ->team)
            {
                if (options.targetKillTeam && targets_remaining == 0 && Player_is_alive(pl))
    ...
    case SCORE_TREASURE:
    {
        ...
        {
            if (!options.zeroSumScoring)
                Score(killer, Rate(Get_Score(killer), TREASURE_SCORE) / 2, treasure->pos,
                      "Treasure:");
            break;
        }
            ...
            if (pl_i->team == treasure->team)
            {
                ...
                if (options.treasureKillTeam)
                    Player_set_state(pl_i, PL_STATE_KILLED);
                     ...
                     (pl_i->team != TEAM_NOT_SET || pl_i->id == killer->id))
            {
