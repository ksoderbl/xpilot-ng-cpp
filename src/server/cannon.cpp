...
void Cannon_update(bool tick)
{
	...
	for (i = 0; i < Num_cannons(); i++)
	{
		...
		if (tick)
		{
			...
			if (!BIT(c->used, HAS_EMERGENCY_SHIELD) && !BIT(c->used, USES_PHASING_DEVICE) && (c->damaged <= 0) && (c->tractor_count <= 0) && rfrac() * 16 < 1)
				Cannon_check_fire(c);

			if (options.itemProbMult > 0 && options.cannonItemProbMult > 0)
			{
...
static void Cannon_aim(cannon_t *c, int weapon, player_t **pl_p, int *dir)
{
	...
	for (i = 0; i < NumPlayers && !ready; i++)
	{
		...
		if (tdist > visualrange && options.survivalScore == 0.0)
			continue;
		...
		case 1:
			...
			/* in cannon dodgers */
			if (tdist < range || options.survivalScore != 0.0)
				ready = true;
			break;
		case 2:
			if (tdist < closest)
		...
		case 3:
			if (tdist < range)
			{
				...
				if ((ddir < (CANNON_SPREAD * 0.5) || ddir > RES - (CANNON_SPREAD * 0.5)) && LENGTH(tdx, tdy) < closest)
				{
					*dir = tdir;
					found = true;
				}
