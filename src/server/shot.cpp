...
static inline bool Player_can_place_mine(Player *pl)
{
    ...
    if (Player_is_phasing(pl))
        return false;
    if (BIT(pl->used, HAS_SHIELD) && !options.shieldedMining)
        return false;
    return true;
}
...
void Delete_shot(int ind)
{
    ...
    case OBJ_PULSE:
        if (shot->id == NO_ID || BIT(shot->obj_status, FROMCANNON))
            break;
        pl = Player_by_id(shot->id);
        if (--pl->num_pulses <= 0)
            pl->num_pulses = 0;
...
void Update_connector_force(ballobject_t *ball)
{
    ...
    damping = -options.ballConnectorDamping * ((pl->vel.x - ball->vel.x) * D.x + (pl->vel.y - ball->vel.y) * D.y);
