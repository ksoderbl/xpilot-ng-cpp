...
void Place_item(Player *pl, int item)
{
    ...
    else
    {
        ...
        if (Num_itemConcs(world) > 0 && rfrac() < options.itemConcentratorProb)
            con = ItemConc_by_index(world, (int)(rfrac() * Num_itemConcs(world)));
        else
            con = nullptr;
        /*
...
void Do_deflector(Player *pl)
{
    ...
    for (i = 0; i < obj_count; i++)
    {
        ...
        if (obj->obj_life <= 0 || obj->mass == 0)
            continue;

        if (obj->id == pl->id)
        {
            if (BIT(obj->obj_status, OWNERIMMUNE) || obj->fuse > 0 || options.selfImmunity)
                continue;
        }
        else
        {
    ...
    for (i = 0; i < NumPlayers; i++)
    {
        ...
        if (Player_is_active(p))
        {
            ...
            if (p->item[ITEM_CLOAK] <= 1)
                ...
                p->forceVisible += damage * pow(0.75, (double)(p->item[ITEM_CLOAK] - 1));
