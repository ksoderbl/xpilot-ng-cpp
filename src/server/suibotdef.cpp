...
static void Robot_set_pointing_direction(Player *pl, double direction)
{
    ...
    if (pl->turnspeed != 0 | pl->turnresistance != 0)
        End_game();
...
bool Robot_evade_shot(Player *pl)
{
    ...
    { /*for .. obj_count*/
        ...
        sqdistance =
            (sqr(delta_velx) + sqr(delta_vely)) * sqr(time_shot_closest) + 2 * (delta_velx * delta_x + delta_vely * delta_y) * time_shot_closest + sqr(delta_x) + sqr(delta_y);
...
#define DD false /* debug */
    ...
    for (i = 0; i < 30; i++)
    { /* allow only limited amount of iterations */

        /* Newton iterations to get the time of impact */
        function =
            -0.25 * sqr_a * sqr(sqr(time)) + sqr_b * sqr(time) + 2 * b_dot_c * time + sqr_c;
        deviation =
            -sqr_a * sqr(time) * time + 2 * sqr_b * time + 2 * b_dot_c;
    ...
    if (DD)
    {
        ...
        printf("test1: %.4f %.4f %.4f\n",
               ...
               +sqr_b * sqr(time) + 2 * b_dot_c * time + sqr_c,
               -sqr_a / 4 * sqr(sqr(time)) + sqr_b * sqr(time) + 2 * b_dot_c * time + sqr_c);
    }
    /*
...
void Robot_attack_player(Player *pl, Player *opponent)
{
    ...
    if (tmp_c >= 0)
    { /* square-root only if number positive*/
...
static void Robot_suibot_play(Player *pl)
{
    ...
    {
        double time;
        time = (pl->velocity - options.maxUnshieldedWallBounceSpeed) / (pl->power / pl->mass);
        time = time * 0.05;
    ...
    for (ship_i = 0; ship_i < NumPlayers; ship_i++)
    {
        ...
        if ((ship->id != pl->id) && Player_is_alive(ship) && ship_dist < ship_dist_closest && (pl->team != ship->team) && ((!BIT(ship->used, HAS_SHIELD)) || World_wrap_length(world, ship->pos.cx - pl->pos.cx, ship->pos.cy - pl->pos.cy) < 8000))
        {
    ...
    {
        direction = World_wrap_cfindDir(world, -closest_opponent->pos.cx + pl->pos.cx,
