...
static bool Really_empty_space(Player *pl, int bx, int by)
{
    ...
    for (i = -1; i <= 1; i++)
    {
        for (j = -1; j <= 1; j++)
        {
            ...
            group = is_inside(pos.cx, pos.cy, hitmask, nullptr);

            /* hack so that robots won't rotate in destination wormholes */
            if (group != NO_GROUP)
            {
                group_t *gp = groupptr_by_id(group);
                if (gp != nullptr && gp->type == WORMHOLE && gp->mapobj_ind == pl->wormHoleDest)
                    group = NO_GROUP;
                else if (gp != nullptr && !options.targetTeamCollision && gp->type == TARGET && gp->team == pl->team)
                    group = NO_GROUP;
            }
...
static inline bool Gravity_is_strong(Player *pl, clpos_t pos, int travel_dir)
{
    ...
    if (sqr(grav.x) + sqr(grav.y) >= 0.5)
    {
        ...
        gravity_dir = MOD2((int)(gdir + 0.5), ANGLE_RESOLUTION);
...
static bool Check_robot_evade(Player *pl, int mine_i, int ship_i)
{
    ...
    stop_dist = ((ANGLE_RESOLUTION * velocity) / (MAX_PLAYER_TURNSPEED * pl->turnresistance) + (velocity * velocity * pl->mass) / (2 * MAX_PLAYER_POWER) + safe_width);
    /*
    ...
    for (dist = 0; dist < stop_dist + BLOCK_SZ / 2; dist += BLOCK_SZ / 2)
    {
        for (i = 0; i < 3; i++)
        {
            ...
            dx = WORLD_WRAP_XBLOCK(world, dx);
            dy = WORLD_WRAP_YBLOCK(world, dy);
...
static void Robotdef_fire_laser(Player *pl)
{
    ...
    dir4 = World_wrap_findDir(world, x4 - x2, y4 - y2);
    dir5 = World_wrap_findDir(world, x5 - x2, y5 - y2);
    if ((dir4 > dir5)
...
static bool Check_robot_target(Player *pl, clpos_t item_pos, int new_mode)
{
    ...
    dx = CLICK_TO_PIXEL(item_pos.cx - pl->pos.cx);
    dx = WORLD_WRAP_DX(world, dx);
    dy = CLICK_TO_PIXEL(item_pos.cy - pl->pos.cy);
    ...
    else
    {
        ...
        item_dir = (int)(idir + 0.5);
        item_dir = MOD2(item_dir, ANGLE_RESOLUTION);
    }
    ...
    pl->power = (Timing(world) ? MAX_PLAYER_POWER : MAX_PLAYER_POWER / 2);
    ...
    if (new_mode == RM_ATTACK || (Timing(world) && new_mode == RM_NAVIGATE))
    {
...
static int Rank_item_value(Player *pl, enum Item itemtype)
{
    ...
    if (itemtype == ITEM_FUEL)
    {
        ...
        else if ((pl->fuel.sum < (Timing(world))
                      ? my_data->fuel_l1
                      : my_data->fuel_l2))
...
static bool Ball_handler(Player *pl)
{
    ...
    for (i = 0; i < Num_treasures(world); i++)
    {
        ...
        {
            dist = World_wrap_length(world, treasure->pos.cx - pl->pos.cx,
        ...
        else if (treasure->team != pl->team && Team_by_index(world, treasure->team)->NumMembers > 0 && !BIT(pl->have, HAS_BALL) && !pl->ball && treasure->have)
        {
            dist = World_wrap_length(world, treasure->pos.cx - pl->pos.cx,
                                     treasure->pos.cy - pl->pos.cy) /
                   CLICK;
    ...
    if (BIT(pl->have, HAS_BALL) || pl->ball)
    {
        ...
        for (i = 0; i < NumPlayers; i++)
        {
            ...
            dist = LENGTH(ball->pos.cx - pl_i->pos.cx,
                   ...
                   CLICK;
            if (pl_i->id != pl->id && Player_is_active(pl_i) && dist < dist_np)
                dist_np = dist;
        }
        closest_treasure = Treasure_by_index(world, closest_tr);
        dbdir = findDir(ball->vel.x, ball->vel.y);
        dtdir = World_wrap_cfindDir(world, closest_treasure->pos.cx - ball->pos.cx,
             ...
             dist += BLOCK_SZ / 2)
        {
            ...
            dx = WORLD_WRAP_XBLOCK(world, dx);
            dy = WORLD_WRAP_YBLOCK(world, dy);
            if (dx < 0 || dx >= world->bwidth_floor || dy < 0 || dy >= world->bheight_floor)
            {
                clear_path = false;
                continue;
            }
        ...
        else
        {
            ...
            return Check_robot_target(pl,
                                      Treasure_by_index(world, closest_tr)->pos,
                                      RM_NAVIGATE);
        }
    }
    else
    {
        ...
        if (closest_ball == NO_IND && closest_ntr_dist < (my_data->robot_count / 10) * BLOCK_SZ)
        {
            SET_BIT(my_data->longterm_mode, FETCH_TREASURE);
            return Check_robot_target(pl,
                                      Treasure_by_index(world, closest_ntr)->pos,
                                      RM_NAVIGATE);
        }
...
static int Robot_default_play_check_map(Player *pl)
{
    ...
    for (j = 0; j < Num_targets(world); j++)
    {
        ...
        if (targ->dead_ticks > 0 || pl->team == targ->team || Team_by_index(world, targ->team)->NumMembers == 0)
            continue;
    ...
    if (fuel_i != NO_IND && (target_dist > fuel_dist || !Team_play(world)) && BIT(my_data->longterm_mode, NEED_FUEL))
    {
                                             ...
                                             int *mine_i, double *mine_dist)
{
    ...
    for (j = 0; j < obj_count; j++)
    {
        ...
        if ((shot->type == OBJ_SHOT || shot->type == OBJ_CANNON_SHOT || shot->type == OBJ_PULSE) && BIT(pl->used, HAS_SHIELD))
            continue;
        ...
        if (shot->type == OBJ_PULSE)
        {
            ...
            if (pulse->id == pl->id && !pulse->pulse_refl)
                continue;
        }
        ...
        if (shot->type == OBJ_TORPEDO ||
            ...
            (shot->type == OBJ_SHOT && !Timing(world) && shot->id != pl->id && shot->id != NO_ID) ||
            (shot->type == OBJ_MINE && shot->id != pl->id) ||
            (shot->type == OBJ_WRECKAGE && !Timing(world)))
        {
            ...
            if ((dx = (int)((CLICK_TO_PIXEL(shot->pos.cx - pl->pos.cx)) + (shot->vel.x - pl->vel.x)),
                 dx = (int)WORLD_WRAP_DX(world, dx), ABS(dx)) < *mine_dist &&
                (dy = (int)((CLICK_TO_PIXEL(shot->pos.cy - pl->pos.cy)) + (shot->vel.y - pl->vel.y)),
                 dy = (int)WORLD_WRAP_DY(world, dy), ABS(dy)) < *mine_dist &&
        ...
        if (shot->type == OBJ_ASTEROID)
        {
            ...
            if (*mine_dist > (wire->wire_size == 1 ? 2 : 4) * BLOCK_SZ &&
                *mine_dist < 8 * BLOCK_SZ &&
                (delta_dir = (pl->dir - World_wrap_cfindDir(world, shot->pos.cx - pl->pos.cx, shot->pos.cy - pl->pos.cy)) < wire->wire_size * (ANGLE_RESOLUTION / 10) || delta_dir > ANGLE_RESOLUTION - wire->wire_size * (ANGLE_RESOLUTION / 10)))
...
static void Robot_default_play(Player *pl)
{
    ...
    if (Player_has_cloaking_device(pl) && pl->fuel.sum > my_data->fuel_l2)
    ...
    if (Player_has_deflector(pl) && !Timing(world))
        Deflector(pl, true);

    if (pl->fuel.sum <= (Timing(world) ? 0 : my_data->fuel_l1))
    ...
    if (pl->fuel.sum < pl->fuel.max * 0.80)
    {
        for (j = 0; j < Num_fuels(world); j++)
        {
            ...
            if ((World_wrap_length(world, pl->pos.cx - fs->pos.cx,
    ...
    if (pl->fuel.sum < (Timing(world) ? my_data->fuel_l1 : my_data->fuel_l3))
    ...
    if (Team_play(world))
    {
        for (j = 0; j < Num_targets(world); j++)
        {
            ...
            if (targ->team == pl->team && targ->damage < TARGET_DAMAGE && targ->dead_ticks >= 0)
            {
                ...
                if (World_wrap_length(world, pl->pos.cx - targ->pos.cx,
    ...
    if (ship_i == NO_IND || enemy_i == NO_IND)
    {
        ...
        for (j = 0; j < NumPlayers; j++)
        {
            ...
            if (j == GetInd(pl->id) || !Player_is_active(ship) || Team_immune(pl->id, ship->id))
                continue;
    ...
    if (ship_dist <= 10 * BLOCK_SZ && pl->fuel.sum <= my_data->fuel_l3 && !Timing(world))
    {
        if (Initiate_hyperjump(pl))
            return;
    }
    ...
    if (enemy_i >= 0)
    {
        ...
        if (!BIT(pl->lock.tagged, LOCK_PLAYER) ||
            (enemy_dist < pl->lock.distance / 2 && (Timing(world) ? (ship->check >= pl->check && ship->round >= pl->round) : 1)) ||
    ...
    if (BIT(pl->lock.tagged, LOCK_PLAYER))
    {
        ...
        ship = Player_by_id(pl->lock.pl_id);
        delta_dir = (int)(pl->dir - World_wrap_cfindDir(world, ship->pos.cx - pl->pos.cx,
                                                        ship->pos.cy - pl->pos.cy));
        ...
        if (!Player_is_active(ship) ||
            ...
            !Detect_ship(pl, ship) ||
            (pl->fuel.sum <= my_data->fuel_l3 && !Timing(world)) ||
            (Timing(world) && (delta_dir < 3 * ANGLE_RESOLUTION / 4 ||
    ...
    if (Timing(world) && !navigate_checked)
    {
        int delta_dir;
        ...
        {
            delta_dir =
                (int)(pl->dir - World_wrap_cfindDir(world, item->pos.cx - pl->pos.cx,
    ...
    if (Team_play(world) && Num_treasures(world) > 0 && world->teams[pl->team].NumTreasures > 0 && !navigate_checked && !BIT(my_data->longterm_mode, TARGET_KILL | NEED_FUEL))
    {
        navigate_checked = true;
        if (Ball_handler(pl))
            return;
