...
Window Widget_window(int widget_desc)
{
    ...
    if ((widget = Widget_pointer(widget_desc)) == nullptr)
        return 0;
...
static void Widget_destroy_viewer(widget_t *w)
{
    ...
    if (v->num_lines > 0 && v->line != nullptr)
        free(v->line);
    v->num_lines = 0;
    v->line = nullptr;
}
...
void Widget_destroy_children(int widget_desc)
{
    ...
    if ((w = Widget_pointer(widget_desc)) != nullptr)
    {
...
void Widget_destroy(int widget_desc)
{
    ...
    if ((w = Widget_pointer(widget_desc)) != nullptr)
    {
        ...
        {
            if ((parent = Widget_pointer(w->parent_desc)) != nullptr && parent->type == WIDGET_FORM)
            {
...
static widget_t *Widget_new(int *descp)
{
    ...
    if (widgets == nullptr || max_widgets <= 0)
    {
        num_widgets = 0;
        max_widgets = 10;
    ...
    else if (num_widgets == 0)
    {
        ...
        widgets[num_widgets].sub = nullptr;
        num_widgets++;
    }
    if (descp != nullptr)
        *descp = num_widgets;
                         ...
                         int width, int height, void *sub)
{
    ...
    if ((widget = Widget_new(&desc)) != nullptr)
    {
        widget->type = type;
        widget->name = name;
...
static int Widget_add_child(int parent_desc, int child_desc)
{
    ...
    if ((parent = Widget_pointer(parent_desc)) == nullptr || (child = Widget_pointer(child_desc)) == nullptr)
    {
        warn("Can't add child widget to parent");
        return NO_WIDGET;
    }
    if (parent->type != WIDGET_FORM || parent->sub == nullptr)
    {
...
static void Widget_draw_input(widget_t *widget, const char *str)
{
    ...
    XDrawString(dpy, widget->window, textGC,
                (widget->width - XTextWidth(textFont, str, (int)strlen(str))) / 2,
                textFont->ascent + (widget->height - (textFont->ascent + textFont->descent)) / 2,
                str, strlen(str));
}
...
static void Widget_draw_slider(widget_t *widget)
{
    ...
    if (widget->type == WIDGET_SLIDER_HORI)
    {
        ...
        block_size = block_max_size * viewer_widget->width / viewerw->real_width;
        block_start = block_max_size * viewerw->visible_x / viewerw->real_width;
        XFillRectangle(dpy, widget->window, buttonGC,
    ...
    else if (widget->type == WIDGET_SLIDER_VERT)
    {
        ...
        block_size = block_max_size * viewer_widget->height / viewerw->real_height;
        block_start = block_max_size * viewerw->visible_y / viewerw->real_height;
        XFillRectangle(dpy, widget->window, buttonGC,
...
static void Widget_viewer_draw_lines(widget_t *widget, int start, int end)
{
    ...
    int text_height = viewerw->font->ascent + viewerw->font->descent,
        i,
        x = 20,
...
static void Widget_draw_viewer(widget_t *widget, XExposeEvent *expose)
{
    ...
    int start,
        ...
        y0,
        y1,
        text_height = viewerw->font->ascent + viewerw->font->descent;
...
{
    Widget_draw_expose(widget_desc, nullptr);
}
...
static void Widget_button(XEvent *event, int widget_desc, bool pressed)
{
    ...
    case WIDGET_BUTTON_MENU:
        ...
        Widget_draw(widget_desc);
        if ((pulldown_widget = Widget_pointer(menuw->pulldown_desc)) == nullptr)
        {
        ...
        if (pressed)
        {
            XMoveWindow(dpy, pulldown_widget->window,
                        ...
                        event->xbutton.y_root - event->xbutton.y + widget->height);
        ...
        else
        {
            ...
            {
                if ((entry_widget = Widget_pointer(pullw->children[i])) == nullptr)
                    continue;
    ...
    case WIDGET_BUTTON_ARROW_RIGHT:
        ...
        if (arroww->inside == false)
            break;
        Widget_draw(widget_desc);
        if (pressed == false && (sub_widget_desc = arroww->widget_desc) != NO_WIDGET && (sub_widget = Widget_pointer(sub_widget_desc)) != nullptr)
        {
            ...
            case WIDGET_INPUT_DOUBLE:
                ...
                if ((widget->type == WIDGET_BUTTON_ARROW_RIGHT) == (dval >= 0))
                {
...
int Widget_event(XEvent *event)
{
    ...
    if (event->type == ButtonRelease)
    {
        if (event->xbutton.button == Button1)
        {
            ...
            for (i = 1; i < num_widgets; i++)
            {
                if ((widget = Widget_pointer(i)) == nullptr)
                    continue;
                switch (widget->type)
                {
                case WIDGET_BUTTON_BOOL:
        ...
        for (i = 1; i < num_widgets; i++)
        {
            ...
            if (widgets[i].window == event->xany.window)
            {
                ...
                case ConfigureNotify:
                    if (widgets[i].name != nullptr && strncmp(widgets[i].name, "popup", 5) == 0)
                    {
                              ...
                              int width, int height)
{
    ...
    {
        if ((parent_widget = Widget_pointer(parent_desc)) == nullptr)
        {
    ...
    if ((formw = XMALLOC(widget_form_t, 1)) == nullptr)
    {
        error("No memory for form widget");
        XDestroyWindow(dpy, window);
        return NO_WIDGET;
    }
    formw->children = nullptr;
                           ...
                           void *user_data)
{
    ...
    if ((parent_widget = Widget_pointer(parent_desc)) == nullptr || parent_widget->type != WIDGET_FORM)
    {
        warn("Widget_create_activate: Invalid parent widget");
        return NO_WIDGET;
    }
    if ((activw = XMALLOC(widget_activate_t, 1)) == nullptr)
    {
                       ...
                       void *user_data)
{
    ...
    if ((boolw = XMALLOC(widget_bool_t, 1)) == nullptr)
    {
                              ...
                              void *user_data)
{
    ...
    if ((menu_widget = Widget_pointer(menu_desc)) == nullptr || menu_widget->type != WIDGET_BUTTON_MENU)
    {
    ...
    if (pulldown_desc == NO_WIDGET)
    {
        ...
        pulldown_desc = Widget_form_window(window, NO_WIDGET,
                                           pull_width, pull_height);
        if ((pulldown_widget = Widget_pointer(pulldown_desc)) == nullptr)
        {
    ...
    else if ((pulldown_widget = Widget_pointer(pulldown_desc)) == nullptr)
    {
        warn("Not a pulldown");
        return NO_WIDGET;
    }
    ...
    if ((entryw = XMALLOC(widget_entry_t, 1)) == nullptr)
    {
    ...
    width = XTextWidth(buttonFont, str, (int)strlen(str)) + (height - (buttonFont->ascent + buttonFont->descent));
                       ...
                       int border, const char *str)
{
    ...
    if ((menuw = XMALLOC(widget_menu_t, 1)) == nullptr)
    {
                      ...
                      void *user_data)
{
    ...
    if ((intw = XMALLOC(widget_int_t, 1)) == nullptr)
    {
                        ...
                        void *user_data)
{
    ...
    if ((colorw = XMALLOC(widget_color_t, 1)) == nullptr)
    {
        error("No memory for int widget");
        return NO_WIDGET;
    }
                         ...
                         void *user_data)
{
    ...
    if ((doublew = XMALLOC(widget_double_t, 1)) == nullptr)
    {
                        ...
                        int border, const char *str)
{
    ...
    if ((labelw = XMALLOC(widget_label_t, 1)) == nullptr)
    {
                               ...
                               int border, int related_desc)
{
    ...
    if ((arroww = XMALLOC(widget_arrow_t, 1)) == nullptr)
    {
                        ...
                        const char *window_name, const char *icon_name)
{
    ...
    popup_desc = Widget_form_window(window, NO_WIDGET,
                                    width, height);
    if ((widget = Widget_pointer(popup_desc)) == nullptr)
        return NO_WIDGET;
    widget->name = "popup";
                          ...
                          int (*callback)(int, void *, const char **))
{
    ...
    const int label_height = textFont->ascent + textFont->descent,
              button_height = 3 * (buttonFont->ascent + buttonFont->descent) / 2,
              label_width = 10 + XTextWidth(textFont, confirm_str,
                                            (int)strlen(confirm_str)),
              button_width = 2 * button_height + XTextWidth(buttonFont, button_str,
                                                            (int)strlen(button_str)),
              label_space = 2 * label_height,
              button_space = button_height,
              popup_height = 2 * label_space + label_height + 2 * button_space + button_height,
    ...
    button_desc = Widget_create_activate(popup_desc,
                                         ...
                                         popup_height - button_height - button_space,
                                ...
                                int border, int viewer_desc)
{
    ...
    if ((sliderw = XMALLOC(widget_slider_t, 1)) == nullptr)
    {
...
static int Widget_viewer_calculate_text(int viewer_desc)
{
    ...
    if (viewerw->num_lines > 0 && viewerw->line != nullptr)
        free(viewerw->line);
    viewerw->line = nullptr;
    viewerw->num_lines = 0;
    viewerw->max_width = 0;
...
static void Widget_resize_viewer(XEvent *event, int ind)
{
    ...
    const int
        ...
        vert_slider_height = height - close_spaced_height - hori_slider_height,
        viewer_width = width - vert_slider_width,
        viewer_height = height - close_spaced_height,
        hori_slider_x = 0,
        hori_slider_y = height - hori_slider_height - close_spaced_height,
                         ...
                         XFontStruct *font)
{
    ...
    if (popup_desc == NO_WIDGET || (popup_widget = Widget_pointer(popup_desc)) == nullptr || popup_widget->type != WIDGET_FORM)
    {
    ...
    if ((viewerw = XMALLOC(widget_viewer_t, 1)) == nullptr)
    {
