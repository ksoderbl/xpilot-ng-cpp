...
static void Talk_refresh(void)
{
	...
	XDrawString(dpy, talkWindow, talkGC,
				XTextWidth(talkFont, talk_str, (int)selection.talk.x1) + TALK_INSIDE_BORDER,
				talkFont->ascent + TALK_INSIDE_BORDER,
...
int Talk_place_cursor(XButtonEvent *xbutton, bool pending)
{
	 ...
	 */
	if (cursor_pos < 0 || x > (int)TALK_WINDOW_WIDTH || y < 0 || y >= (int)TALK_WINDOW_HEIGHT)
	{
		if (Button == 1 && pending)
		{
...
{
	selection.draw.x1 = selection.draw.x2 = selection.draw.y1 = selection.draw.y2 = 0;
...
void Talk_cut_from_messages(XButtonEvent *xbutton)
{
	...
	else if (xbutton->type == ButtonRelease)
	{
		...
		/* swap order? */
		if (c2.y < c1.y || (c2.y == c1.y && (c2.x_off - c1.x_off < 0 || (c2.x < c1.x && (c2.x_off - c1.x_off == 0)))))
		{
		 ...
		 */
		if (c1.y == c2.y && (c1.str_index > c2.str_index || (c1.str_index == c2.str_index && c1.x_off == c2.x_off)))
		{
		...
		if (c2.str_index == 0)
		{
			...
			else
			{
				...
				if (c1.y == c2.y && c1.str_index >= c2.str_index && c1.x_off == 1)
				{
					Add_pending_messages();
					Selection_set_state();
					return;
