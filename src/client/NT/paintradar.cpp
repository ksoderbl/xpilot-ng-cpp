 ...
 *      Dick Balaska
 *
 * Copyright (C) 2000-2001 Juha Lindström
 *
...
static void Windows_copy_sliding_radar(double xf, double yf)
{
	...
	slidingradar_y = (RadarHeight - (int)(selfPos.y * yf + 0.5) - 1 + RadarHeight / 2) % RadarHeight;
	...
	WinXBltPixToWin(radarPixmap2, radarWindow,
					slidingradar_x, 1,
					256 - slidingradar_x, slidingradar_y, 0,
	...
	else
	{
		...
		for (xi = 0; xi < 256; xi++)
		{
			...
			for (yi = 0; yi < (int)RadarHeight; yi++)
			{
				...
				if (start != -1 && (!vis || yi == (int)RadarHeight - 1 || visibleColorChange))
				{
					if (end > start)
					{
						segments[nsegment].x1 = xp;
		...
		for (xoff = min.x; xoff <= max.x; xoff++)
		{
			for (yoff = min.y; yoff <= max.y; yoff++)
			{
				...
				for (j = 0; j < polygons[i].num_points; j++)
				{
					...
					poly[j].y = RadarHeight - ((y * RadarHeight) / Setup->height);
				}
