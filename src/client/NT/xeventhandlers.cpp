...
int ClientMessage_event(XEvent *event)
{
    ...
    if (cmev->message_type == ProtocolAtom && cmev->format == 32 && (unsigned)cmev->data.l[0] == KillAtom)
    {
...
void ButtonPress_event(XEvent *event)
{
    ...
    {
        if (clData.pointerControl && !clData.talking && event->xbutton.button <= MAX_POINTER_BUTTONS)
