...
void Widget_destroy(int widget_desc)
{
	...
	if ((w = Widget_pointer(widget_desc)) != NULL)
	{
		...
		{
			if ((parent = Widget_pointer(w->parent_desc)) != NULL && parent->type == WIDGET_FORM)
			{
...
static int Widget_add_child(int parent_desc, int child_desc)
{
	...
	{
		incr = 4;
		form->children = malloc((form->num_children + incr) * sizeof(*form->children));
	}
	else
	{
		incr = 4 + form->num_children / 2;
		form->children = realloc(form->children,
								 (form->num_children + incr) * sizeof(*form->children));
	}
	if (form->children == NULL)
	{
		form->num_children = 0;
		...
		for (i = 1; i < num_widgets; i++)
		{
			...
			if (widgets[i].window == event->xany.window)
			{
				...
				case ConfigureNotify:
					if (widgets[i].name != NULL && strncmp(widgets[i].name, "popup", 5) == 0)
					{
