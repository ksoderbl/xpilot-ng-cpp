...
int Net_start(void)
{
	...
	for (retries = 0;;)
	{
		if (retries == 0 || (time(NULL) - last) > 1)
		{
			 ...
			 */
			if ((retries < 5 && Send_shape(shipShape) == -1) || Packet_printf(&wbuf, "%c", PKT_PLAY) <= 0 || Client_power() == -1
#ifdef SOUND
		...
		while (cbuf.len <= 0 && sock_readable(&rbuf.sock) != 0)
		{
			...
			if (rbuf.ptr[0] != PKT_RELIABLE)
			{
				...
				else if (rbuf.ptr[0] == PKT_START)
				{
					...
					{
						if (rbuf.ptr[0] == PKT_END && rbuf.ptr + 5 < rbuf.buf + rbuf.len && rbuf.ptr[5] == PKT_RELIABLE)
						{
							rbuf.ptr += 5;
							break;
						}
...
static void Net_lag_measurement(long key_ack)
{
	...
	for (i = 0; i < KEYBOARD_STORE; i++)
	{
		if (keyboard_change[i] == key_ack && keyboard_acktime[i] == -1)
		{
