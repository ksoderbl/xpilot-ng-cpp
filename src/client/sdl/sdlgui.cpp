...
#define GRAV(x0, y0, x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, x7, y7) \
    ...
    glVertex2i(x + x3, y + y3);                                              \
    set_alphacolor(c2);                                                      \
    glVertex2i(x + x4, y + y4);                                              \
    glVertex2i(x + x5, y + y5);                                              \
    set_alphacolor(c0);                                                      \
    glVertex2i(x + x6, y + y6);                                              \
    glVertex2i(x + x7, y + y7);                                              \
    glEnd();
...
void Gui_paint_polygon(const xp_polygon_t &polygon, int i, int xoff, int yoff)
{
    ...
    glTranslatef(polygon.points[0].x * clData.scale +
                 ...
                 polygon.points[0].y * clData.scale +
                     rint((yoff * Setup->height - world.y) * clData.scale),
                 0);
                    ...
                    int shield, int deflector, int eshield)
{
    ...
    if (texturedShips)
    {
        ...
        if (cloak || phased)
            Image_paint_rotated(img, x, y, dir, (color & 0xffffff00) + ((color & 0x000000ff) / 2));
        else
            Image_paint_rotated(img, x, y, dir, color);
    }
    else
    {
        glEnable(GL_BLEND);
        ...
        if (sobj->life_time > 0)
        {
            if (loopsSlow % 3)
            {
                ...
                if (wrap(&x, &y))
                {
                    ...
                    if (score_object_texs[i].tex_list.empty() || score_object_texs[i].text != sobj->msg)
                    {
                        free_string_texture(&score_object_texs[i]);
                        draw_text(&mapfont, scoreObjectColorRGBA, CENTER, CENTER, x, y, sobj->msg, true, &score_object_texs[i], false);
                    }
                    else
                        disp_text(&score_object_texs[i], scoreObjectColorRGBA, CENTER, CENTER, x, y, false);
                }
            }
...
static void Paint_HUD_items(int hud_pos_x, int hud_pos_y)
{
    ...
    for (i = 0; i < NUM_ITEMS; i++)
    {
        ...
        if (num >= 0)
        {
            ...
            HUDprint(&gamefont, hudItemsColorRGBA, RIGHT, UP, horiz_pos - ITEM_SIZE - BORDER, draw_height - vert_pos - ITEM_SIZE, "%s", str);
...
void Paint_HUD(void)
{
    ...
    if (hudRadarEnemyColorRGBA || hudRadarOtherColorRGBA || hudRadarObjectColorRGBA)
    {
        hudRadarMapScale = (double)Setup->width / (double)RadarWidth;
        Paint_hudradar(
            hudRadarScale,
    ...
    if (hudColorRGBA && (fuelTime > 0.0 || fuelSum < fuelNotify))
    {
        ...
        {
            if (!HUD_texs[tex_index].tex_list.empty())
                free_string_texture(&HUD_texs[tex_index]);
            strlcpy(hud_texts[tex_index], str, 50);
        }
        if (HUD_texs[tex_index].tex_list.empty())
            render_text(&gamefont, str, &HUD_texs[tex_index]);
        disp_text(&HUD_texs[tex_index], hudColorRGBA, LEFT, DOWN, hud_pos_x + hudSize - HUD_OFFSET + BORDER, hud_pos_y - (hudSize - HUD_OFFSET + BORDER), true);
    ...
    if (hudColorRGBA)
    {
        for (i = 0, j = 0; i < MAX_SCORE_OBJECTS; i++)
        {
            ...
            if (sobj->hud_msg_len > 0)
            {
                ...
                disp_text(&HUD_texs[tex_index], hudColorRGBA, CENTER, DOWN, hud_pos_x, hud_pos_y - (hudSize - HUD_OFFSET + BORDER + j * HUD_texs[tex_index].height), true);
                j++;
            }
        }
        ...
        if (time_left > 0)
        {
            ...
            if (HUD_texs[tex_index].tex_list.empty())
                render_text(&gamefont, str, &HUD_texs[tex_index]);
            disp_text(&HUD_texs[tex_index], hudColorRGBA, RIGHT, DOWN, hud_pos_x - hudSize + HUD_OFFSET - BORDER, hud_pos_y + hudSize + HUD_OFFSET + BORDER, true);
        }
        ...
        if (strlen(mods))
        {
            ...
            disp_text(&HUD_texs[tex_index], hudColorRGBA, RIGHT, UP, hud_pos_x - hudSize + HUD_OFFSET - BORDER, hud_pos_y - hudSize + HUD_OFFSET - BORDER, true);
        }
        ...
        if (autopilotLight)
        {
            ...
            disp_text(&HUD_texs[tex_index], hudColorRGBA, RIGHT, DOWN, hud_pos_x, hud_pos_y + hudSize + HUD_OFFSET + BORDER + HUD_texs[tex_index].height * 2, true);
        }
    }
        ...
        for (j = 0; j <= maxMessages - 1; ++j)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                    case BmsBall:
                        msg_color = &msgScanBallColorRGBA;
                        break;
                    case BmsSafe:
                        msg_color = &msgScanSafeColorRGBA;
                        break;
                    case BmsCover:
                        msg_color = &msgScanCoverColorRGBA;
                        break;
                    case BmsPop:
                        msg_color = &msgScanPopColorRGBA;
                        break;
                    default:
                        msg_color = &messagesColorRGBA;
                        break;
