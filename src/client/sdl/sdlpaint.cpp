    ...
    if (scoresChanged)
    {
        ...
        if (y != scoreEntryRect.y)
        {
            sdl_window_resize(&scoreListWin, scoreListWin.w,
                              scoreEntryRect.y + scoreEntryRect.h + 2 * SCORE_BORDER);
...
void Paint_score_entry(int entry_num, Other *other, bool is_team)
{
    ...
    if (showUserName)
        sprintf(label, "%s=%s@%s",
                other->nick_name.c_str(), other->user_name.c_str(), other->host_name.c_str());
    else
    {
        if (BIT(Setup->mode, TIMING))
        {
        ...
        if (BIT(Setup->mode, TEAM_PLAY))
            sprintf(label, "%c%s %-15s%s",
                    other->mychar, scoreStr, other->nick_name.c_str(), lifeStr);
        else
