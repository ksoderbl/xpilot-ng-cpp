...
static GLWidget *scraptarget = nullptr;
...
GLWidget *Init_EmptyBaseGLWidget(void)
{
    ...
    tmp->bounds.w = 0;
    tmp->bounds.h = 0;
    tmp->wid_info = nullptr;
    tmp->Draw = nullptr;
    tmp->Close = nullptr;
    tmp->SetBounds = nullptr;
    tmp->button = nullptr;
    tmp->buttondata = nullptr;
    tmp->motion = nullptr;
    tmp->motiondata = nullptr;
    tmp->hover = nullptr;
    tmp->hoverdata = nullptr;
    tmp->list = nullptr;
...
static void clear_eventTarget(GLWidget *widget)
{
    ...
    if (!widget)
    {
        error("nullptr passed to clear_eventTarget!");
        return;
    }

    for (i = 0; i < NUM_MOUSE_BUTTONS; ++i)
        if (widget == clicktarget[i])
            clicktarget[i] = nullptr;
    if (widget == hovertarget)
        hovertarget = nullptr;
    if (widget == scraptarget)
        scraptarget = nullptr;
}

/* only supposed to take care of deletes done on behalf of the
...
void Close_Widget(GLWidget **widget)
{
    ...
    if (!widget)
    {
        error("nullptr passed to Close_Widget!");
        return;
    }
    if (!(*widget))
    {
        error("pointer passed to Close_Widget points to nullptr !");
        return;
    }
...
void SetBounds_GLWidget(GLWidget *widget, SDL_Rect *b)
{
    if (!widget)
    {
        error("nullptr widget passed to SetBounds_GLWidget!");
        return;
    }
    if (!b)
    {
        error("nullptr bounds passed to SetBounds_GLWidget!");
        return;
    }

    if (widget->SetBounds)
        widget->SetBounds(widget, b);
    else
    {
        widget->bounds.x = b->x;
        widget->bounds.y = b->y;
...
static void hover_optionWidget(int over, Uint16 x, Uint16 y, void *data)
{
    ...
    if (over)
    {
        if (!data)
        {
            error("nullptr option passed to hover_optionWidget\n");
            return;
        }
        ...
        if ((help = Option_get_help(opt)))
        {
            ...
            if (eternalLoop)
                error("hover_optionWidget: string parse never ends! (infinite loop prevented)\n");
...
static void option_callback(void *tmp, const char *value)
{
    ...
    {
        error("Faulty parameter to option_callback: opt is a nullptr pointer!");
        return;
    }
...
GLWidget *Init_OptionWidget(xp_option_t *opt, Uint32 *fgcolor, Uint32 *bgcolor)
{
    if (!opt)
    {
        error("Faulty parameter to Init_DoubleChooserWidget: opt is a nullptr pointer!");
        return nullptr;
    }
...
GLWidget *FindGLWidgeti(GLWidget *widget, Uint16 x, Uint16 y)
{
    ...
    if ((x >= widget->bounds.x) && (x <= (widget->bounds.x + widget->bounds.w)) && (y >= widget->bounds.y) && (y <= (widget->bounds.y + widget->bounds.h)))
    {
...
void load_textscrap(const std::string &text)
{
    ...
    scraptarget = nullptr;
    scrap = (char *)realloc(scrap, text.size() + 1);
    if (!scrap)
    {
    ...
    strcpy(scrap, text.c_str());
    for (cp = scrap, i = 0; i < (int)text.size(); ++cp, ++i)
    {
        if (*cp == '\n')
            *cp = '\r';
    }
...
static void Paint_ButtonWidget(GLWidget *widget)
{
    ...
    if (!widget)
        return;
    if (!(wid_info = (ButtonWidget *)(widget->wid_info)))
        return;
...
static void Close_ScrollbarWidget(GLWidget *widget)
{
    ...
    delete (ScrollbarWidget *)widget->wid_info;
    widget->wid_info = nullptr;
}
                               ...
                               void *poschangedata)
{
    ...
    tmp->wid_info = new ScrollbarWidget();
    if (!(tmp->wid_info))
    {
        delete tmp;
        error("Failed to malloc in Init_ScrollbarWidget");
    ...
    AppendGLWidgetList(&(tmp->children), wid_info->slide);
    return tmp;
}
/*************************/
/*   End: ScrollbarWidget*/
...
static void Close_LabelWidget(GLWidget *widget)
{
    ...
    delete (LabelWidget *)widget->wid_info;
    widget->wid_info = nullptr;
}
...
static void Paint_LabelWidget(GLWidget *widget)
{
    ...
    x = wid_info->align == LEFT ? tmp->bounds.x : wid_info->align == CENTER ? tmp->bounds.x + tmp->bounds.w / 2
                                                                            : tmp->bounds.x + tmp->bounds.w;
    y = wid_info->valign == DOWN ? tmp->bounds.y : wid_info->valign == CENTER ? tmp->bounds.y + tmp->bounds.h / 2
...
GLWidget *Init_LabelWidget(const char *text, Uint32 *fgcolor, Uint32 *bgcolor, int align, int valign)
{
    ...
    if (!render_text(&gamefont, text, &(wid_info->tex)))
    {
        delete wid_info;
        delete tmp;
    ...
    tmp->bounds.w = wid_info->tex.width;
    tmp->bounds.h = wid_info->tex.height;
                                        ...
                                        void *actiondata, bool start_state)
{
    ...
    if (!ontex || ontex->tex_list.empty() || !offtex || offtex->tex_list.empty())
    {
    ...
    tmp->wid_info = new LabeledRadiobuttonWidget();
    if (!(tmp->wid_info))
    {
        delete tmp;
        error("Failed to malloc in Init_LabeledRadiobuttonWidget");
...
static string_tex_t *BoolChooserWidget_ontex = nullptr;
static string_tex_t *BoolChooserWidget_offtex = nullptr;
...
static void Close_BoolChooserWidget(GLWidget *widget)
{
    ...
    {
        delete BoolChooserWidget_ontex;
        BoolChooserWidget_ontex = nullptr;
        delete BoolChooserWidget_offtex;
        BoolChooserWidget_offtex = nullptr;
    }

    delete (BoolChooserWidget *)widget->wid_info;
    widget->wid_info = nullptr;
}
                                 ...
                                 void (*callback)(void *tmp, const char *value), void *data)
{
    ...
    if (!value)
    {
        error("Faulty parameter to Init_BoolChooserWidget: value is a nullptr pointer!");
        return nullptr;
    }
    if (!name || !strlen(name))
    {
    ...
    {
        BoolChooserWidget_ontex = new string_tex_t();
        if (!BoolChooserWidget_ontex)
        {
        ...
        BoolChooserWidget_offtex = new string_tex_t();
        if (!BoolChooserWidget_offtex)
        {
            delete BoolChooserWidget_ontex;
            BoolChooserWidget_ontex = nullptr;
    ...
    tmp->wid_info = new BoolChooserWidget();
    if (!(tmp->wid_info))
    {
        delete tmp;
...
static void Close_IntChooserWidget(GLWidget *widget)
{
    ...
    delete (IntChooserWidget *)widget->wid_info;
    widget->wid_info = nullptr;
}
...
static void Paint_IntChooserWidget(GLWidget *widget)
{
    ...
    if (!widget)
    {
        error("Paint_IntChooserWidget: argument is nullptr!");
        return;
    }
    ...
    if (wid_info->direction > 0)
        --(wid_info->direction);
    else if (wid_info->direction < 0)
        ++(wid_info->direction);
                                ...
                                Uint32 *bgcolor, void (*callback)(void *tmp, const char *value), void *data)
{
    ...
    if (!value)
    {
        error("Faulty parameter to Init_IntChooserWidget: value is a nullptr pointer!");
        return nullptr;
    }
    if (!(name) || !strlen(name))
    {
    ...
    tmp->wid_info = new IntChooserWidget();
    if (!(tmp->wid_info))
    {
        delete tmp;
        error("Failed to malloc in Init_IntChooserWidget");
    ...
    if (*(wid_info->value) <= wid_info->minval)
        ((ArrowWidget *)(wid_info->leftarrow->wid_info))->locked = true;
    ...
    if (*(wid_info->value) >= wid_info->maxval)
        ((ArrowWidget *)(wid_info->rightarrow->wid_info))->locked = true;
...
static void Close_DoubleChooserWidget(GLWidget *widget)
{
    ...
    delete (DoubleChooserWidget *)widget->wid_info;
    widget->wid_info = nullptr;
}
...
static void Paint_DoubleChooserWidget(GLWidget *widget)
{
    ...
    if (!widget)
    {
        error("Paint_DoubleChooserWidget: argument is nullptr!");
        return;
    }
                                   ...
                                   void (*callback)(void *tmp, const char *value), void *data)
{
    ...
    if (!value)
    {
        error("Faulty parameter to Init_DoubleChooserWidget: value is a nullptr pointer!");
        return nullptr;
    }
    if (!(name) || !strlen(name))
    {
    ...
    tmp->wid_info = new DoubleChooserWidget();
    if (!(tmp->wid_info))
    {
        delete tmp;
        error("Failed to malloc in Init_DoubleChooserWidget");
...
static void Paint_ColorChooserWidget(GLWidget *widget)
{
    ...
    if (!widget)
    {
        error("Paint_ColorChooserWidget: argument is nullptr!");
        return;
    }
...
static void action_ColorChooserWidget(void *data)
{
    ...
    {
        error("action_ColorChooserWidget: argument is nullptr!");
        return;
    }
    ...
    if (wid_info->expanded)
    {
        if (wid_info->mod)
        {
            ...
            error("action_ColorChooserWidget: Color mod widget mysteriously missing!");
    ...
    else
    {
        if (!(wid_info->mod))
        {
            ...
            error("action_ColorChooserWidget: Color mod widget mysteriously present!");
                                  ...
                                  void (*callback)(void *tmp, const char *value), void *data)
{
    ...
    if (!value)
    {
        error("Faulty parameter to Init_ColorChooserWidget: value is a nullptr pointer!");
        return nullptr;
    }
    if (!(name) || !strlen(name))
    {
...
static void Paint_ColorModWidget(GLWidget *widget)
{
    ...
    if (!widget)
    {
        error("Paint_ColorModWidget: argument is nullptr!");
        return;
    }
...
static void Callback_ColorModWidget(void *tmp, const char *value)
{
    ...
    {
        error("Callback_ColorModWidget: argument is nullptr!");
        return;
    }
    ...
    *(wid_info->value) = ((wid_info->red) << 24) | ((wid_info->green) << 16) | ((wid_info->blue) << 8) | (wid_info->alpha);

    snprintf(str, 10, "#%02X%02X%02X%02X", wid_info->red, wid_info->green, wid_info->blue, wid_info->alpha);
    str[9] = '\0';

    if (wid_info->callback)
        wid_info->callback(wid_info->data, str);
}
                              ...
                              void (*callback)(void *tmp, const char *value), void *data)
{
    ...
    if (!value)
    {
        error("Faulty parameter to Init_ColorModWidget: value is a nullptr pointer!");
        return nullptr;
    }
    tmp = Init_EmptyBaseGLWidget();
    if (!tmp)
    ...
    tmp->bounds.h = wid_info->redpick->bounds.h + wid_info->greenpick->bounds.h + wid_info->bluepick->bounds.h + wid_info->alphapick->bounds.h;
...
bool ListWidget_Append(GLWidget *list, GLWidget *item)
{
    ...
    if (!list)
    {
        error("ListWidget_Append: *list is nullptr!");
        return false;
    }
    ...
    if (!item)
    {
        error("ListWidget_Append: *item is nullptr");
        return false;
    }
...
bool ListWidget_Prepend(GLWidget *list, GLWidget *item)
{
    ...
    if (!list)
    {
        error("ListWidget_Prepend: *list is nullptr!");
        return false;
    }
    if (list->WIDGET != LISTWIDGET)
    {
    ...
    if (!item)
    {
        error("ListWidget_Prepend: *item is nullptr");
        return false;
    }
...
bool ListWidget_Insert(GLWidget *list, GLWidget *target, GLWidget *item)
{
    ...
    if (!list)
    {
        error("ListWidget_Insert: *list is nullptr!");
        return false;
    }
    if (list->WIDGET != LISTWIDGET)
    {
    ...
    if (!item)
    {
        error("ListWidget_Insert: *item is nullptr");
        return false;
    }
...
bool ListWidget_Remove(GLWidget *list, GLWidget *item)
{
    ...
    if (!list)
    {
        error("ListWidget_Remove: *list is nullptr!");
        return false;
    }
    if (list->WIDGET != LISTWIDGET)
    {
    ...
    if (!item)
    {
        error("ListWidget_Remove: *item is nullptr");
        return false;
    }
...
bool ListWidget_SetScrollorder(GLWidget *list, bool order)
{
    ...
    if (!list)
    {
        error("ListWidget_SetScrollorder: *list is nullptr!");
        return false;
    }
    if (list->WIDGET != LISTWIDGET)
    {
...
int ListWidget_NELEM(GLWidget *list)
{
    ...
    if (!list)
    {
        error("ListWidget_NELEM: *list is nullptr!");
        return -1;
    }
    if (list->WIDGET != LISTWIDGET)
    {
...
GLWidget *Init_ListWidget(Uint16 x, Uint16 y, Uint32 *bg1, Uint32 *bg2, Uint32 *highlight_color, ListWidget_ver_dir_t v_dir, ListWidget_hor_dir_t h_dir, ListWidget_direction direction, bool reverse_scroll)
{
    GLWidget *tmp;
    ListWidget *wid_info;
...
static void ScrollPaneWidget_poschange(GLfloat pos, void *data)
{
    ...
    if (!data)
    {
        error("nullptr data to ScrollPaneWidget_poschange!");
        return;
    }
...
static void SetBounds_ScrollPaneWidget(GLWidget *widget, SDL_Rect *b)
{
    ...
    for (i = 0; i < 2; ++i)
    {
            ...
            (widget->bounds.w < wid_info->content->bounds.w))
        {
            if (!wid_info->hori_scroller)
            {
                if (!AppendGLWidgetList(&(widget->children),
                                        (wid_info->hori_scroller = Init_ScrollbarWidget(false, 0.0f, 1.0f, SB_HORISONTAL, ScrollPaneWidget_poschange, widget))))
                {
            ...
            (widget->bounds.h < wid_info->content->bounds.h))
        {
            ...
            {
                if (!AppendGLWidgetList(&(widget->children),
                                        (wid_info->vert_scroller = Init_ScrollbarWidget(false, 0.0f, 1.0f, SB_VERTICAL, ScrollPaneWidget_poschange, widget))))
                {
...
GLWidget *Init_ScrollPaneWidget(GLWidget *content)
{
    ...
    wid_info->hori_scroller = nullptr;
    wid_info->vert_scroller = nullptr;
...
void MainWidget_ShowMenu(GLWidget *widget, bool show)
{
    ...
    else
    {
        ...
        if (hovertarget)
            hover_optionWidget(0, 0, 0, nullptr);
    }
}
...
static void SetBounds_ConfMenuWidget(GLWidget *widget, SDL_Rect *b)
{
    ...
    if (!widget)
    {
        error("SetBounds_ConfMenuWidget: tried to change bounds on nullptr ConfMenuWidget!");
        return;
    }
    if (widget->WIDGET != CONFMENUWIDGET)
    {
    ...
    if (!b)
    {
        error("SetBounds_ConfMenuWidget: tried to set nullptr bounds on ConfMenuWidget!");
        return;
    }
...
static void Paint_ConfMenuWidget(GLWidget *widget)
{
    ...
    if (!widget)
    {
        error("Paint_ConfMenuWidget: tried to paint nullptr ConfMenuWidget!");
        return;
    }
...
static void SetBounds_LabelButtonWidget(GLWidget *widget, SDL_Rect *b)
{
    ...
    if (!widget)
    {
        error("SetBounds_LabelButtonWidget: tried to change bounds on nullptr ImageButtonWidget!");
        return;
    }
    ...
    if (!b)
    {
        error("SetBounds_LabelButtonWidget: tried to set nullptr bounds on LabelButtonWidget!");
        return;
    }
